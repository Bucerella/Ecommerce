import 'package:flutter/material.dart';

class Constants {
  static String fav = "Favorites";
  static String basket = "Basket";
  static String add = "Add";
  static String empty = "Empty";
  static String order = "Order";
  static String ecommerce = "E-commerce";
  static String successful = "Successful";
  static String addedSuccessfully = "added successfully!";
  static String products = "Products";
  static String totalPrice = "Total Price";
  static String goToProducts = "Go to Products";
  static String emptyFavMsg = "Your favorites are empty yet, add something now.";
  static String emptyBasketMsg = "Your cart is empty, add something now!";

  static const Color appColor = Color(0xff990000);
}